...
def plt_support_(clf, X_, y_, kernel, c):
    ...
    cs = ax.contour(X_tmp, Y_tmp, Z_rbf, [0], colors='blue', linewidths=1)
    set_ax_gray(ax)

    ax.scatter(X_[pos, 0], X_[pos, 1], label='1', color='green')
    ax.scatter(X_[neg, 0], X_[neg, 1], label='0', color='r')

    ax.scatter(X_[clf.support_, 0], X_[clf.support_, 1], marker='o', c='', edgecolors='g', s=150,
               label='support_vectors')

    ax.legend()
    ax.set_title('{} kernel, C={}, gamma=0.8'.format(kernel, c))
    plt.show()


path = 'watermelon_3a.txt'
